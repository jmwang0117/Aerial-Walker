...
struct MappingParameters {
  ...
  double esdf_slice_height_, visualization_truncate_height_, virtual_ceil_height_, ground_height_, ground_judge_;
  bool show_esdf_time_, show_occ_time_;
  
  bool global_sensing_;
  /* active mapping */
...
private:
  ...
  void raycastProcess(vector<Eigen::Vector3d> project_points, int points_cnt);
...
inline int SDFMap::getGroundOccupancy() {
  return ground_occupied_flag;
}
...
inline Eigen::Vector3i SDFMap::posToIndex(const Eigen::Vector3d &pos)
{
  Eigen::Vector3i id;
  id(0) = floor((pos(0) - mp_.map_origin_(0)) * mp_.resolution_inv_);
  id(1) = floor((pos(1) - mp_.map_origin_(1)) * mp_.resolution_inv_);
  id(2) = floor((pos(2) - mp_.map_origin_(2)) * mp_.resolution_inv_);
  return id;
}
