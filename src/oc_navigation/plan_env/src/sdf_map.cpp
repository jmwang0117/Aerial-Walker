...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  node_.param("sdf_map/ground_judge", mp_.ground_judge_, 1.0);
  node_.param("sdf_map/global_sensing", mp_.global_sensing_, false);
  ...
  cout << "ground_judge: " << mp_.ground_judge_ << endl;
...
void SDFMap::SegmentGroundPoints(){
...
void SDFMap::projectDepthImage() {
  ...
  //cout << "proj_points_cnt: " << md_.proj_points_cnt << endl;
  SegmentGroundPoints();
...
void SDFMap::raycastProcess(vector<Eigen::Vector3d> project_points, int points_cnt) {
  // if (md_.proj_points_.size() == 0)
  if (points_cnt == 0) return;
...
void SDFMap::updateOccupancyCallback(const ros::TimerEvent& /*event*/) {
  ...
  // raycastProcess(md_.ground_points_to_pub_, md_.ground_points_cnt);
  raycastProcess(md_.obstacle_points_to_pub_, md_.obstacle_points_cnt);
  //ros::Time T2 = ros::Time::now();
  //cout << "raycast_process_time: " << (T2 - T1).toSec() <<endl;
  ...
  for (int x = min_cut(0); x <= max_cut(0); ++x)
    for (int y = min_cut(1); y <= max_cut(1); ++y)
      for (int z = min_cut(2); z <= max_cut(2); ++z) {
        ...
        if (md_.occupancy_buffer_[toAddress(x, y, z)] < mp_.clamp_min_log_ - 1e-3 && md_.occupancy_buffer_inflate_[toAddress(x, y, z)] == 0)
         {
          Eigen::Vector3d pos;
          indexToPos(Eigen::Vector3i(x, y, z), pos);
          if (pos(2) > mp_.visualization_truncate_height_) continue;
